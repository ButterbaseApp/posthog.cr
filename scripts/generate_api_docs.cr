...
class APIDocGenerator
  ...
  # Exclude internal utility types
  EXCLUDED_TYPES = ["Utils", "VERSION"]
  ...
  def generate
    ...
    posthog_module = @root.program.types.try(&.find { |t| t.name == "PostHog" })
    return puts "Error: PostHog module not found" unless posthog_module

    types = collect_types(posthog_module)
  ...
  private def render_type(type : DocType) : String
    String.build do |io|
      ...
      if loc = type.locations.try(&.first?)
        ...
        io << "(https://github.com/watzon/posthog.cr/blob/main/" << loc.filename << "#L" << loc.line_number << ")*\n\n"
      end
  ...
  private def render_method(io : IO, method : DocType::Method, separator : String)
    ...
    if loc = method.location
      io << "*[View source](https://github.com/watzon/posthog.cr/blob/main/"
  ...
  private def generate_index(types : Array(DocType))
    content = String.build do |io|
      io << "# API Reference\n\n"
      io << "Complete API documentation for PostHog Crystal SDK, auto-generated from source code.\n\n"
